...
txt_file_path = filepaths['MT-10'] # Same as batch_file_path from big_master_backtests_generator.py
set_file_path = filepaths['MT-10Presets']
...
symbol = 'AUDUSD'
period = 'M15' # This is the ea chart period, not the rolling_runner update period. That's the reset_period
...
while True:
    try:
        ...
        if start_cond: 
            # Kill all currently running metatrader apps (called terminal.exe)
            print('killing process...')
            [ p for p in psutil.process_iter() if 'terminal.exe' in p.name() if 'OANDA - MetaTrader - 10' in p.exe()][0].kill()
...
only_positive_results=0'''.format(long_back_1,long_back_2,short_back_1,
    ...
    time.sleep(59.99) #Sleep (delay program) for 1 minute